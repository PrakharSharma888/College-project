...
contract CryptoDevs is Ownable{
    ...
    function createProposal(uint256 _nftTokenId) external returns(uint){
    ...
    function voteOnProposals(uint256 proposalIndex, Vote vote) external activeProposalOnly(proposalIndex){
        
        Proposal storage proposal = proposals[proposalIndex];
        uint256 voterNFTBalance = 2;
        uint numVotes = 0;
    ...
    function executeProposal(uint proposalIndex) external inactiveProposalOnly(proposalIndex){
        Proposal storage proposal = proposals[proposalIndex];
